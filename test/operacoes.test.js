...
describe('Suíte de Testes Fraca para 50 Operações Aritméticas', () => {
  ...
  test('6. deve calcular a raiz quadrada e tratar casos de limite e erro', () => {
  ...
  test('11. deve encontrar o valor máximo em um array', () => { 
    ...
    // Buscando apenas a parte final da string:
    expect(() => maximoArray([])).toThrow('possui valor máximo.'); 
  });

  test('12. deve encontrar o valor mínimo em um array', () => { 
    expect(minimoArray([10, 2, 100])).toBe(2); 
    // Buscando apenas a parte final da string:
    expect(() => minimoArray([])).toThrow('possui valor mínimo.'); 
  });
  ...
  test('47. deve calcular a mediana de arrays (ímpares, pares, desordenados e vazios)', () => { 

  // --- Casos ímpares (mantidos, matam mutantes da linha 109) ---
  expect(medianaArray([1, 2, 3, 4, 5])).toBe(3); 
  expect(medianaArray([3, -10, 1])).toBe(1);
  expect(medianaArray([20, 3, 10, 1, 2])).toBe(3);
  ...
  expect(medianaArray([40, -10, 20, 30])).toBe(25);


  expect(medianaArray([10, 20])).toBe(15);

  expect(medianaArray([7, 1, 5, 3])).toBe(4);

  expect(() => medianaArray([])).toThrow('possui mediana.'); 
});
